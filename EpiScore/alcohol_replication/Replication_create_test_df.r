...
df = data[, 1:10]
colnames(df) = paste0("ID", 1:ncol(df))
...
head(pheno_example)
write.csv(pheno_example, "pheno_example.csv", row.names = F)
write.csv(pheno, "pheno_full.csv", row.names = F)
...
cpgs = cpgs[-1,]
cpgs$Predictor = "Alcohol Consumption Score"
colnames(cpgs) = c("CpG", "Beta", "Predictor") 
write.csv(cpgs, "weights.csv", row.names = F)
...
write.csv(out, "results_check.csv")
