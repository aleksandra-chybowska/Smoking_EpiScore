...
scores = colnames(pheno[2:(length(colnames(pheno))-5)])
...
for (score in scores) {
  ...
  r <- cor(pheno$alcunitsupw, pheno[score], use="pairwise.complete.obs")
  out[which(out$score == sc), "r"] <- r

  # Incremental DNAm R2
  null <- summary(lm(pheno$alcunitsupw ~ pheno$age + pheno$sex))$r.squared
  full <- summary(lm(pheno$alcunitsupw ~ pheno$age + pheno$sex + pheno[,score]))$r.squared 
