...
methdir <- "methylation_complete_no_age_sex"
filename <- "pack_years_17833_complete_genetic.csv"
type <- "complete_genetic"
...
# Export residualized
write.table(x = t(as.matrix(as.numeric(scale(pack_years$pack_years_clean)))),
file = paste0(datadir, "pack_years_", type, "_", sample_size, "_not_residualized.csvphen"),
              quote = F, sep = ",", row.names = F, col.names = F)

print(paste("Pheno file ready:", 
        paste0(datadir, "pack_years_", type, "_", sample_size, "_not_residualized.csvphen")))
...
cl <- makeCluster(6, outfile=paste0(datadir, "parallel_test.txt")) # Let's start with 6 since they use up a lot of RAM
...
foreach(i=1:22, .packages = "data.table") %dopar% { 
  ...
  design.resid <- model.matrix(~ as.factor(Batch), data=pack_years)
